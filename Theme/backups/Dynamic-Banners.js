...
* @Date:   2016-12-01 15:09:07
* @Last Modified by:   BirdyOz
* @Last Modified time: 2016-12-21 11:40:19
*
* Add dynamic banners to Moodle course, based on breadcrumbs
* Banner contains:
*     - Custom logo, based on category
*     - Code(s)
*     - Decriptive title
*     - Audience (if provided)
*     - Year
*     - 'In development' badging
*/
...
$(document).ready(function() {
    ...
    // Extract division
    var division = $(breadcrumbs[2]).text();

    // If one of GOTAFE's named divisions
    if (division === 'Courses' || division === 'NCEE' || division === 'NCDE' || division === 'HR') {
        logo = division;
    } else {
        logo = "Main";
    }
    ...
    // Extract course short description from breadcrumbs.  It is set as the first anchor with a title attribute within ul.breadcrumb
    var description = $("ul.breadcrumb a[title] span").first().text();
